...
var gseriesIntroMY = map[byte]byte{
	...
	'W': 'N',
	// '5': G-series (G20 3er etc.) from MY2019 ('K'); before that it was the
	//      5 Series F10/F11. e.g. WBA5R7C0XLFH66853 (vin[9]='L'=2020) → G20 330i.
	'5': 'K',
}
...
var bmwTypeKeys = map[byte]bmwModelEntry{
	...
	// 3 Series G20/G21 (most common; also G22 4er, G01 X3); before that it was
	// the 5 Series F10/F11 (see gseriesIntroMY['5'] + fseriesAltKeys['5']).
	// e.g. WBA5R7C0XLFH66853 → G20 330i xDrive.
...
func decodeVIN(vin string) (chassis, model, engine, body string, powerKW int) {
	...
	// VIN[9] is the ISO 3779 model-year character (position 10, 1-indexed; VIN[8]
	// is the check digit) and disambiguates which generation the car belongs to.
	// If the model year predates the G-series introduction for this key, fall
	// back to the F-series meaning.
	typeKey := vin[3]
	myChar := vin[9]
